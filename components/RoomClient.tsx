...
export default function RoomClient({ roomId }: { roomId: string }) {
  ...
  return (
    <div className="flex h-screen flex-col bg-slate-100">
      {/* Header */}
      <header className="z-10 flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white/95 px-4 py-3 shadow-sm backdrop-blur-sm">
        <div className="flex min-w-0 items-center gap-3">
          <button
            type="button"
            onClick={() => router.push("/")}
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:bg-slate-50"
            title="Back to home"
          >
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-500">
              <span>Code</span>
              <span className="rounded-md bg-slate-100 px-1.5 py-px font-mono font-semibold tracking-widest text-slate-700">
                {room.code}
              </span>
              <button
                type="button"
                onClick={onCopyCode}
                className={`rounded-md px-1.5 py-px text-[11px] font-medium transition ${copied ? "bg-emerald-100 text-emerald-700" : "text-brand-600 hover:bg-brand-50"}`}
              >
                {copied ? "✓ Copied" : "Copy"}
              </button>
            </div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden md:flex-row">
        ...
        {/* Sidebar */}
        <aside className="flex h-72 w-full flex-col border-t border-slate-200 bg-white md:h-auto md:w-96 md:border-l md:border-t-0">
          <div className="flex-1 overflow-y-auto">
            <RestPointPicker
              restPoint={room.rest_point}
              canEdit={isRoomOwner}
              picking={pickingRestPoint}
              onTogglePick={() => setPickingRestPoint((p) => !p)}
            ...
            <StopsPanel
              ...
              picking={pickingStop}
              busy={stopBusy}
              onTogglePick={() => setPickingStop((p) => !p)}
          ...
          <div className="flex h-64 flex-col border-t border-slate-100 md:h-72">
