      ...
      <header className="sticky top-0 z-10 border-b border-slate-200/80 bg-white/80 backdrop-blur-sm">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onLogout} className="btn-secondary py-1.5 text-xs">
              Log out
            </button>
          </div>
        </div>
      ...
      <main className="mx-auto max-w-5xl px-6 py-10">
        ...
        <div className="mt-10 grid grid-cols-3 gap-4">
          {[
            ...
            { icon: "🗺️", title: "Shared destination", desc: "Host sets a meeting point for the group" },
            { icon: "💬", title: "Group chat", desc: "Coordinate without switching apps" },
          ].map((f) => (
            <div key={f.title} className="rounded-xl border border-slate-200 bg-white/60 px-4 py-3 text-center">
